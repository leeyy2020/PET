...
class ErnieMLMCriterion(paddle.nn.Layer):
    ...
    def forward(self, prediction_scores, masked_lm_labels, masked_lm_scale=1.0):
        # print('prediction_scores', prediction_scores.shape, prediction_scores)
...
class AlbertLMPredictionHead(nn.Layer):
    ...
    def __init__(self,
        ...
        # print('embedding_weights', embedding_weights.shape, embedding_weights)
    ...
    def forward(self, hidden_states, masked_positions=None):
        ...
        # print('self.decoder_weight', self.decoder_weight.shape, self.decoder_weight)
        # print('self.decoder_bias', self.decoder_bias.shape, self.decoder_bias)
        hidden_states = paddle.tensor.matmul(
            hidden_states, self.decoder_weight,
            transpose_y=True) + self.decoder_bias  # this is different for Albert
...
class AlbertForPretraining(AlbertForMaskedLM):
    def __init__(self, *args, **kwargs):
        ...
        # print(self.transformer.embeddings)
        # self.albert_predictionhead = AlbertLMPredictionHead(
        #     self.transformer.config["embedding_size"],
        #     self.transformer.config["hidden_size"],
        #     self.transformer.config["vocab_size"],
        #     self.transformer.config["hidden_act"],
        #     embedding_weights=self.transformer.embeddings.word_embeddings.weight
        # )
    ...
    def forward(self,
        ...
        with paddle.static.amp.fp16_guard():
            ...
            # print("masked_number:{}".format(masked_number))
            
            if masked_number not in [1, 2, 3, 4]:
                raise ValueError("Illegal masked_number:{}".format(masked_number))
            ...
            if masked_number in [1, 2, 3, 4]:
                ...
                prediction_scores = paddle.reshape(prediction_scores, [-1, vocab_size])
                filtered_scores = paddle.gather(prediction_scores, new_masked_positions, axis=0)
            else:
                # print(masked_number)
                # Prediction scores: [bs, max_seq_len, vocab_size]
                # print(masked_positions)
                # assert bool(paddle.equal_all(paddle.expand(masked_positions[0], masked_positions.shape), masked_positions))
                # mask_start_index = int(masked_positions[0][0])
                # mask_end_index = mask_start_index + masked_number
                
                # filtered_scores = paddle.slice(prediction_scores, [1], [mask_start_index], [mask_end_index])            
                # filtered_scores = paddle.reshape(filtered_scores, [-1, filtered_scores.shape[-1]])
                pass
    ...
    def predict(self,
        ...
        prediction_logits = self.forward(
            ...
            attention_mask=attention_mask,
            masked_positions=masked_positions)
        # print('prediction logits shape', prediction_logits.shape)
        # print('prediction logits', prediction_logits)
