...
local_log_path="/home/Code/PET_Rdrop/log_pet_${pretrained_model}_turn${turn}"
submit_dir="/home/Code/PET_Rdrop/log_pet_${pretrained_model}/submit"
#task_name="iflytek"
#task_name="tnews"
#task_name="eprstmt"
#task_name="bustm"
...
# if [[ ${language_model} == "albert-chinese-xxlarge" && ${task_name} == "iflytek" ]]; then
# 	batch_size=(4)
# fi
...
function train() {
	...
	strategy="bs${bs}_lr${lr}_patternId${id}"
...
function train_wrapper() {
    ...
    for id in ${pattern_id[@]}; do
		echo "[strat training] ${task_name} ${lr} ${bs} ${id} ${language_model}"
		train ${task_name} ${lr} ${bs} ${id} ${language_model}
    done
	done
	done
}
