...
def do_train(args):
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for step, batch in enumerate(train_data_loader, start=1):
            ...
            src_ids_1 = batch[4]
            token_type_ids_1 = batch[5]
            masked_positions_1 = batch[6]
            masked_lm_labels_1 = batch[7]

            # print("shape_1:")
            # print(src_ids_1.shape)
            # print(token_type_ids_1.shape)
            # print(masked_positions_1.shape)
            # print(masked_lm_labels_1.shape)
            ...
            max_len_1 = src_ids_1.shape[1]
            new_masked_positions_1 = []
            for bs_index, mask_pos in enumerate(masked_positions_1.numpy()):
                for pos in mask_pos:
                    new_masked_positions_1.append(bs_index * max_len_1 + pos)
            new_masked_positions_1 = paddle.to_tensor(
                np.array(new_masked_positions_1).astype('int32'))
            ...
            prediction_scores_1 = model(
                input_ids=src_ids_1,
                token_type_ids=token_type_ids_1,
                masked_positions=new_masked_positions_1)
            ...
            global_step += 1
            if global_step % 1 == 0 and rank == 0:
                print(
                    "global step %d, epoch: %d, batch: %d, loss: %.5f, ce_loss: %.5f, kl_loss: %.5f, speed: %.2f step/s"
                    % (global_step, epoch, step, loss, ce_loss, kl_loss,
